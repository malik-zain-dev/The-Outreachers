...
def test_process_one_sends_email_when_eligible(monkeypatch):
    ...
    db.users.find_one.return_value = {"id": "u1", "email": "user@example.com", "first_name": "Ritik"}
    monkeypatch.setattr(svc, "_is_eligible", AsyncMock(return_value=True))
    monkeypatch.setattr(
        svc,
        "_template_vars",
        AsyncMock(return_value={"first_name": "Ritik", "plan_price": "$29", "open_rate": "41.0", "open_rate_position": "above"}),
    )
