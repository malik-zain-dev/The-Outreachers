"""
System-level email templates for EmaReach AI.
...
BRAND_NAME = "EmaReach AI"
SUPPORT_EMAIL = "contact@emareach.com"
# Logo color theme: blue-to-purple gradient + soft blue accent
PRIMARY_BLUE = "#0ea5e9"       # Bright blue (left side of logo)
PRIMARY_PURPLE = "#7c3aed"     # Deep purple (right side of logo)
PRIMARY_COLOR = "#6366f1"      # Indigo (mid-gradient) for solid fallbacks
PRIMARY_LIGHT = "#eff6ff"      # Very light blue (soft AI accent)
FOOTER_COLOR = "#64748b"
LINK_COLOR = "#6366f1"
# Gradient for header and CTAs (135deg blue → purple)
...
def verification_otp(code: str, expires_minutes: int = 15) -> Tuple[str, str, str]:
    ...
    subject = "Verify your email — EmaReach AI"
    plain = (
        f"Hi,\n\n"
        f"Your verification code is: {code}\n\n"
        f"This code expires in {expires_minutes} minutes. If you didn't create an account with EmaReach AI, you can safely ignore this email.\n\n"
        f"Best regards,\n{BRAND_NAME}"
    )
    content = f"""
...
def login_2fa_otp(code: str, expires_minutes: int = 5) -> Tuple[str, str, str]:
    ...
    subject = "Your login code — EmaReach AI"
    plain = (
        f"Hi,\n\n"
    ...
    content = f"""
        <p style="margin:0 0 16px;font-size:16px;line-height:1.6;color:#334155;">Hi,</p>
        <p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#334155;">Use the code below to complete your sign-in to EmaReach AI.</p>
        <div style="background:{PRIMARY_LIGHT};border:2px solid {PRIMARY_BLUE};border-radius:8px;padding:20px 24px;text-align:center;margin:0 0 24px;">
            <span style="font-size:28px;font-weight:700;letter-spacing:6px;color:{PRIMARY_PURPLE};">{code}</span>
        </div>
        <p style="margin:0 0 8px;font-size:14px;color:#64748b;">This code expires in {expires_minutes} minutes.</p>
...
def password_reset(reset_url: str, expires_hours: int = 1) -> Tuple[str, str, str]:
    ...
    subject = "Reset your password — EmaReach AI"
    plain = (
        f"Hi,\n\n"
        f"You requested a password reset for your EmaReach AI account.\n\n"
    ...
    content = f"""
        <p style="margin:0 0 16px;font-size:16px;line-height:1.6;color:#334155;">Hi,</p>
        <p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#334155;">You requested a password reset for your EmaReach AI account. Click the button below to choose a new password.</p>
    ...
    """
    html = _wrap_html(content, preheader="Reset your EmaReach AI password")
    return subject, plain, html
...
def mailbox_password_reset(reset_url: str, inbox_email: str, expires_hours: int = 1) -> Tuple[str, str, str]:
    ...
    subject = "Set or reset mailbox password — EmaReach AI"
    plain = (
        f"Hi,\n\n"
...
def campaign_started(campaign_name: str) -> Tuple[str, str, str]:
    ...
    subject = "Campaign started — EmaReach AI"
    plain = (
        f"Your campaign \"{campaign_name}\" has started and is now sending emails.\n\n"
        f"Log in to EmaReach AI to monitor progress and replies.\n\n"
        f"Best regards,\n{BRAND_NAME}"
    )
    content = f"""
...
def campaign_completed(campaign_name: str) -> Tuple[str, str, str]:
    ...
    subject = "Campaign completed — EmaReach AI"
    plain = (
        f"Your campaign \"{campaign_name}\" has completed. All contacts have been sent.\n\n"
        f"Log in to EmaReach AI to view results and replies.\n\n"
        f"Best regards,\n{BRAND_NAME}"
    )
    content = f"""
...
def reply_notification() -> Tuple[str, str, str]:
    ...
    subject = "New reply to your campaign — EmaReach AI"
    plain = (
        "A prospect has replied to your email.\n\n"
        "Log in to EmaReach AI to read the reply and continue the conversation.\n\n"
        f"Best regards,\n{BRAND_NAME}"
    )
    content = """
        <p style="margin:0 0 8px;font-size:18px;font-weight:600;color:{PRIMARY_PURPLE};">New reply</p>
        <p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#334155;">A prospect has replied to your campaign email. Check your inbox in EmaReach AI to read the reply and respond.</p>
...
def ticket_reply_notification(
    ...
    subject = "Support replied to your ticket — EmaReach AI"
...
def health_alert(domain_name: str, message: str) -> Tuple[str, str, str]:
    ...
    subject = "Domain health alert — EmaReach AI"
    plain = (
        f"Domain health alert: {message}\n\n"
        "Log in to EmaReach AI to review your domain health and consider pausing campaigns if needed.\n\n"
        f"Best regards,\n{BRAND_NAME}"
    )
    content = f"""
...
def weekly_report(sent: int, opened: int, replied: int) -> Tuple[str, str, str]:
    ...
    subject = "Your weekly report — EmaReach AI"
    plain = (
        "Your weekly summary (last 7 days):\n\n"
        f"• Emails sent: {sent}\n"
        f"• Opened: {opened}\n"
        f"• Replied: {replied}\n\n"
        "Log in to EmaReach AI to see full details and manage your campaigns.\n\n"
        f"Best regards,\n{BRAND_NAME}"
    )
    content = f"""
...
def connection_test_email() -> Tuple[str, str, str]:
    ...
    subject = "Connection successful — EmaReach AI"
    plain = (
        "Hi,\n\n"
        "Your email connection test was successful. Your inbox is activated and ready to use with EmaReach AI.\n\n"
        f"Best regards,\n{BRAND_NAME}"
    )
    content = f"""
        <p style="margin:0 0 16px;font-size:16px;line-height:1.6;color:#334155;">Hi,</p>
        <p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#334155;">Your email connection test was successful. Your inbox is activated and ready to use with EmaReach AI.</p>
    ...
    """
    html = _wrap_html(content, preheader="Your EmaReach AI inbox is ready")
    return subject, plain, html
...
def workspace_team_invitation(
    ...
    *,
    ...
    subject = "You've been added to a workspace — EmaReach AI"
    if invitee_has_account:
        plain = (
            f"Hi,\n\n"
            f"{inviter_plain} has added you to their EmaReach AI workspace as a team member.\n\n"
        ...
        <p style="margin:0 0 16px;font-size:16px;line-height:1.6;color:#334155;">Hi,</p>
        <p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#334155;"><strong>{inviter_html}</strong> has added you to their EmaReach AI workspace as a team member.</p>
        ...
        <p style="margin:0 0 8px;font-size:18px;font-weight:600;color:{PRIMARY_PURPLE};">Workspace invitation</p>
        <p style="margin:0 0 16px;font-size:16px;line-height:1.6;color:#334155;">Hi,</p>
        <p style="margin:0 0 16px;font-size:16px;line-height:1.6;color:#334155;"><strong>{inviter_html}</strong> has added you to their EmaReach AI workspace as a team member.</p>
...
def subscription_payment_failed_notification(
    ...
    *,
    ...
    """
    subject = "Your subscription payment didn't go through — EmaReach AI"
    ...
    plain = (
        "Hi,\n\n"
        "We were notified that a charge for your EmaReach AI subscription could not be completed. "
