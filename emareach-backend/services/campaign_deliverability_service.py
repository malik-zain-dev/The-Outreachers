...
class CampaignDeliverabilityService:
    ...
    async def _run_unit_check(
        ...
        *,
        ...
        outbound_message_id = f"<deliverability-{probe_id}@emareach.local>"
        try:
    ...
    async def _build_probe_content(
        ...
        HTML may include an invisible `<!--emareach-probe:...-->` comment; inbox/spam matching still
        ...
        if send_body_type == "html" and marker and "<!--emareach-probe:" not in rendered_body:
            rendered_body = rendered_body.rstrip() + f"\n<!--emareach-probe:{marker}-->"
