...
DEFAULT_CHANNELS = {
    "contact": "emareach-contact",
    "ticket": "emareach-ticket",
    "new-user": "emareach-new-user",
}
...
async def notify_contact_submission(
    ...
    """Send new contact form submission to emareach-contact."""
    lines = [
...
async def notify_ticket_created(
    ...
    """Send new ticket created notification to emareach-ticket."""
    lines = [
...
async def notify_new_user_signup(email: str, user_id: str) -> bool:
    """Send new user registration to emareach-new-user."""
    text = "\n".join([
        "*New user sign up*",
        f"*Email:* {email}",
        f"*User ID:* {user_id}",
