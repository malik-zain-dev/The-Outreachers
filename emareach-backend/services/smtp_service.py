...
class SMTPService:
    ...
    async def _get_inbox_config(self, inbox_id: str) -> Dict:
        ...
        # emareach = Emareach-managed SendGrid: use global env API key instead of per-inbox credentials
        if provider == "emareach":
...
{html_body}
    ...
    async def send_email_via_smtp(
        ...
        if domain_sending_provider == "sendgrid":
            if inbox_provider in ("sendgrid", "emareach"):
    ...
    async def test_smtp_connection(self, inbox_id: str) -> bool:
        ...
        try:
            ...
            # emareach = Emareach-managed SendGrid; resolve to underlying provider for connection test
            if provider == "emareach":
    ...
    def _get_app_notification_config(self) -> Optional[Dict[str, Any]]:
        ...
        from_name = os.getenv("NOTIFICATION_FROM_NAME", "EmaReach AI")
        api_key = os.getenv("SENDGRID_API_KEY")
    ...
    async def _send_app_notification_via_sendgrid(
        ...
        payload = {
            "personalizations": [{"to": [{"email": to_email}]}],
            "from": {"email": config["from_email"], "name": config.get("from_name") or "EmaReach"},
            "subject": subject,
            "content": content,
        }
        if attachments:
