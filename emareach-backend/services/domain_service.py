...
class DomainService:
    ...
    async def verify_dns_records_dns_only(self, domain_id: str) -> Dict:
        ...
        # EmaReach / Email Infra SPF: TXT at root matching stored TXT value
        try:
            ...
            if infra_spf_txt:
                ...
                logging.warning(
                    f"[DNS-ONLY-EMAREACH-SPF] TXT records at {ema_spf_host}: {len(txt_records)} found"
                )
                for record in txt_records:
                    if record.strip() == infra_spf_txt.strip():
                        ...
                        logging.warning("[DNS-ONLY-EMAREACH-SPF] EmaReach SPF verified at root")
                        break
        except Exception as e:
            logging.error(f"[DNS-ONLY-EMAREACH-SPF] Error verifying EmaReach SPF for {domain_name}: {e}")

        # EmaReach / Email Infra DKIM: TXT at mail._domainkey.<domain> matching stored TXT value
        try:
            email_infra = domain.get("email_infra") or {}
            ...
            if infra_dkim_txt:
                ...
                logging.warning(
                    f"[DNS-ONLY-EMAREACH-DKIM] TXT records at {ema_dkim_host}: {len(txt_records)} found"
                )
                for record in txt_records:
                    if record.strip() == infra_dkim_txt.strip():
                        ...
                        logging.warning("[DNS-ONLY-EMAREACH-DKIM] EmaReach DKIM verified via mail._domainkey")
                        break
        except Exception as e:
            logging.error(f"[DNS-ONLY-EMAREACH-DKIM] Error verifying EmaReach DKIM for {domain_name}: {e}")

        # EmaReach / Email Infra DMARC: TXT at _dmarc.<domain> matching stored TXT value
        try:
            email_infra = domain.get("email_infra") or {}
            ...
            if infra_dmarc_txt:
                ...
                logging.warning(
                    f"[DNS-ONLY-EMAREACH-DMARC] TXT records at {ema_dmarc_host}: {len(txt_records)} found"
                )
                for record in txt_records:
                    if record.strip() == infra_dmarc_txt.strip():
                        ...
                        logging.warning("[DNS-ONLY-EMAREACH-DMARC] EmaReach DMARC verified at _dmarc")
                        break
        except Exception as e:
            logging.error(f"[DNS-ONLY-EMAREACH-DMARC] Error verifying EmaReach DMARC for {domain_name}: {e}")
