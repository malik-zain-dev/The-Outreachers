...
SUPPORTED_DNS_PROVIDERS = ("cloudflare", "godaddy", "namecheap", "route53")
...
def _expected_tracking_cname_target() -> str:
    expected = (os.getenv("TRACKING_CNAME_TARGET") or "track.emareach.com").strip().lower().rstrip(".")
    return expected
...
def _derive_email_infra_mailbox_password(user_id: str, domain_id: str, email: str) -> str:
    ...
    Mirrors logic in `emareach-backend/routes/inboxes.py`.
    """
...
def _validate_dns_provider_credentials(provider: str, payload: dict) -> Dict[str, str]:
    ...
    if p == "route53":
        ...
        if not access_key or not secret_key:
            raise HTTPException(
                status_code=400,
                detail="aws_access_key_id and aws_secret_access_key are required for route53.",
            )
        creds = {
            "aws_access_key_id": access_key,
            "aws_secret_access_key": secret_key,
            "aws_region": (payload.get("aws_region") or "us-east-1").strip(),
        }
        if (payload.get("aws_session_token") or "").strip():
            creds["aws_session_token"] = (payload.get("aws_session_token") or "").strip()
        if (payload.get("hosted_zone_id") or "").strip():
            creds["hosted_zone_id"] = (payload.get("hosted_zone_id") or "").strip()
        return creds
...
async def sync_domain_to_provider(domain_id: str, current_user: dict = Depends(get_current_user)):
    ...
    This is useful when:
    - Domain was created with a different provider (e.g., EmaReach)
...
async def delete_email_infra_for_domain(
    ...
    """
    Remove EmaReach DNS from this domain: delete the domain in Email Infra (when provisioned)
    ...
    if not infra_meta.get("enabled"):
        raise HTTPException(status_code=400, detail="EmaReach DNS is not configured for this domain.")
