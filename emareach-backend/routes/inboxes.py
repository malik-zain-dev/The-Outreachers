...
async def create_inbox(inbox: Inbox, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # For emareach smtp_provider (Emareach-managed SendGrid + Email Infra enabled), ensure and link one domain mailbox id.
        if (
            inbox_dict.get("sender_type") == "smtp"
            and inbox_dict.get("smtp_provider") == "emareach"
            ...
            if not domain_id:
                raise HTTPException(
                    status_code=400,
                    detail="Domain is required for EmaReach inboxes when Email Infra is enabled.",
                )
