...
const METRICS = [
  { value: "AI",   label: "Powered campaigns", valueClass: "text-cyan-500 dark:text-cyan-400",       cardClass: "bg-cyan-500/8 border-cyan-500/20"    },
  ...
  { value: "Auto", label: "Follow-up sequences",valueClass: "text-violet-500 dark:text-violet-400",   cardClass: "bg-violet-500/8 border-violet-500/20" },
...
function TypewriterOutcome() {
  ...
  return (
    <span className="bg-gradient-to-r from-cyan-500 via-blue-500 to-violet-500 dark:from-cyan-400 dark:via-blue-400 dark:to-violet-400 bg-clip-text text-transparent">
      {displayed}
      <span className="animate-pulse text-cyan-500 dark:text-cyan-400 not-italic">|</span>
    </span>
  );
}
      ...
      {isLive ? (
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className="text-[10px] text-muted-foreground/60">
            <span className="font-bold text-cyan-600 dark:text-cyan-400">{leadsCount}</span> leads
          </span>
          <span className="text-[10px] text-muted-foreground/60">
...
function AICampaignCard() {
  ...
  return (
    <div className="relative w-full max-w-lg mx-auto lg:max-w-none">
      ...
      <div className="absolute -inset-8 rounded-3xl blur-3xl opacity-30 dark:opacity-60 pointer-events-none bg-gradient-to-br from-cyan-500/40 via-transparent to-violet-500/40" />
      ...
      <div className="relative rounded-2xl border border-border/70 bg-card/95 backdrop-blur-xl shadow-2xl overflow-hidden ring-1 ring-foreground/5 dark:ring-white/5">
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border/60 bg-muted/20">
          ...
          <div className="flex-1 mx-3 h-5 rounded-md bg-muted/60 flex items-center justify-center">
            <span className="text-[10px] font-medium text-muted-foreground/50">
              AI Campaign Dashboard — Emareach
            </span>
          </div>
          <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-cyan-500/12 border border-cyan-500/25">
            <Sparkles className="w-2.5 h-2.5 text-cyan-500 dark:text-cyan-400" />
            <span className="text-[9px] text-cyan-600 dark:text-cyan-300 font-black">AI POWERED</span>
          </div>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2.5">
            <div className="rounded-xl bg-cyan-500/8 border border-cyan-500/20 p-3">
              <div className="flex items-center gap-1.5 mb-1">
                <Mail className="w-3 h-3 text-cyan-500 dark:text-cyan-400" />
                <span className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground/70">Campaigns</span>
              </div>
              <span className="text-2xl font-black text-cyan-600 dark:text-cyan-400 tabular-nums">12</span>
            ...
            <div className="rounded-xl bg-violet-500/8 border border-violet-500/20 p-3">
              ...
              <span className="text-xl font-black text-violet-600 dark:text-violet-400 tabular-nums leading-tight">{leadsReached.toLocaleString()}</span>
          ...
          <div className="rounded-xl bg-violet-500/5 border border-violet-500/15 p-3">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-[9px] font-black uppercase tracking-wider text-violet-600 dark:text-violet-400">AI personalization</span>
            </div>
            <p className="text-[10px] text-muted-foreground/80 leading-relaxed font-mono">
              <span className="text-foreground/70">Hi </span>
              <span className="text-cyan-600 dark:text-cyan-400 font-semibold">{"{{first_name}}"}</span>
              <span className="text-foreground/70">, I noticed </span>
              <span className="text-violet-600 dark:text-violet-400 font-semibold">{"{{company}}"}</span>
      ...
      {/* Floating badges */}
      <div className="absolute -top-4 -left-4 text-white text-[10px] font-black px-3.5 py-1.5 rounded-full whitespace-nowrap shadow-lg shadow-cyan-500/30 border-2 border-background bg-gradient-to-br from-cyan-500 to-blue-600">
        <CheckCircle2 className="inline w-3 h-3 mr-1 -mt-0.5" /> 10× more revenue
      </div>
      <div className="absolute -bottom-4 -right-4 text-white text-[10px] font-black px-3.5 py-1.5 rounded-full whitespace-nowrap shadow-lg shadow-violet-500/30 border-2 border-background bg-gradient-to-br from-violet-500 to-indigo-500">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10">
        <div className="grid lg:grid-cols-[1fr_1.08fr] gap-14 lg:gap-20 xl:gap-28 items-center">
          ...
          <div className="max-w-xl mx-auto lg:mx-0">
            ...
            >
              Emareach helps businesses create high-converting AI email campaigns in minutes.
