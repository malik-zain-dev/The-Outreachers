...
/** Last two labels (e.g. emareach.com) — same heuristic as elsewhere in the app. */
...
/**
 * CNAME "Name" / Host at the **registrable** DNS zone (e.g. emareach.com).
 * For apex `example.com` + prefix `cloud` → `cloud`.
 * For `cloud.emareach.com` + `center` → `center.cloud` (full host `center.cloud.emareach.com`).
 */
...
export default function DomainsPage() {
  ...
  const handleCreateEmailInfra = async (domainId: string) => {
    ...
    try {
      ...
      toast.success("EmaReach DNS created for this domain.");
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : "Failed to create EmaReach DNS for this domain";
      toast.error(message);
    } finally {
  ...
  const handleDeleteEmailInfra = async (domain: Domain) => {
    ...
    const confirmed = await confirmDialog({
      title: "Remove EmaReach DNS",
      description: `This removes EmaReach DNS from Email Infra for "${domain.domain}". SendGrid DNS and verification are unchanged. If sending was set to Email Infra, it will switch back to SendGrid.`,
      variant: "destructive",
    });
    if (!confirmed) return;
    ...
    try {
      ...
      toast.success("EmaReach DNS removed");
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : "Failed to remove EmaReach DNS";
      toast.error(message);
    } finally {
  ...
  return (
    ...
    <div className="space-y-6">
                    ...
                    filteredDomainRows.map((row) => {
                      ...
                      <Fragment key={domain.id}>
                        ...
                        >
                          ...
                          {/* If EmaReach DNS is not enabled, show SendGrid DNS metrics directly in the main row */}
                        ...
                        {/* SendGrid DNS sub-row, shown only when EmaReach DNS is enabled for this domain */}
                        {domain.email_infra?.enabled && (
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-lg border bg-muted/30 p-3 font-mono text-sm">
              <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1">
                ...
                <span className="text-muted-foreground">Value</span>
                <span>{(dnsRecords?.tracking?.cname_target || "track.emareach.com").replace(/\.$/, "")}</span>
              </div>
            </div>
            {(() => {
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <Button
                ...
                onClick={() => {
                  void navigator.clipboard.writeText(
                    (dnsRecords?.tracking?.cname_target || "track.emareach.com").replace(/\.$/, "")
                  );
                  toast.success("Value copied to clipboard");
                }}
              >
      ...
      <Dialog open={dnsProviderDialogOpen} onOpenChange={setDnsProviderDialogOpen}>
        <DialogContent className="w-[calc(100vw-2rem)] max-w-lg sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Connect your DNS provider so EmaReach can add SPF, DKIM, DMARC, MX, and CNAME records automatically.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            ...
            {dnsProvider === "route53" ? (
              <>
                <div className="space-y-1.5">
                  ...
                  <Input
                    value={dnsProviderCredentials.aws_access_key_id || ""}
                    onChange={(e) => setDnsProviderCredentials((p) => ({ ...p, aws_access_key_id: e.target.value }))}
                    placeholder="AKIA..."
                    disabled={savingDnsProvider}
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="password"
                    value={dnsProviderCredentials.aws_secret_access_key || ""}
                    onChange={(e) =>
                      setDnsProviderCredentials((p) => ({ ...p, aws_secret_access_key: e.target.value }))
                    }
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">AWS region (optional)</label>
                  <Input
                    value={dnsProviderCredentials.aws_region || ""}
                    onChange={(e) => setDnsProviderCredentials((p) => ({ ...p, aws_region: e.target.value }))}
                    placeholder="us-east-1"
                    disabled={savingDnsProvider}
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium">Hosted zone ID (optional)</label>
                  <Input
                    value={dnsProviderCredentials.hosted_zone_id || ""}
                    onChange={(e) => setDnsProviderCredentials((p) => ({ ...p, hosted_zone_id: e.target.value }))}
                    placeholder="Z123456..."
                    disabled={savingDnsProvider}
                  />
                </div>
      ...
      <HelpLinks
        slugs={[
          "add-verify-sending-domain-emareach",
