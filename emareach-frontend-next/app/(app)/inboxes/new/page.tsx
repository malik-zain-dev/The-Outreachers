...
export default function NewInboxPage() {
  ...
  const [smtpProvider, setSmtpProvider] = useState<"custom" | "emareach">("emareach");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "bulk" && senderType === "smtp") {
      ...
      if (smtpProvider === "custom") {
        toast.error(
          "Custom SMTP is not supported for bulk creation. Each SMTP account requires unique credentials. Please use EmaReach Provider for bulk inbox creation."
        );
        return;
      }
            ...
            {senderType === "smtp" && mode === "single" && (
              <div className="space-y-6">
                    ...
                    <div className="p-4 rounded-lg bg-amber-500/10 border border-amber-500/20 flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          ...
                          providers will reject this. Use EmaReach Provider for domain-level sending.
                        </p>
                      </div>
                    </div>
                  )}
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={smtpProvider}
                    onValueChange={(v: "custom" | "emareach") => setSmtpProvider(v)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="emareach">EmaReach Provider</SelectItem>
                      <SelectItem value="custom">Custom SMTP</SelectItem>
                    </SelectContent>
                  </Select>
                  {smtpProvider === "emareach" && (
                    <p className="text-xs text-muted-foreground">
                      Uses the system-wide EmaReach configuration; no credentials required.
                    </p>
                  )}
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="emareach">⭐ EmaReach Provider (Recommended)</SelectItem>
                    </SelectContent>
                  </Select>
                  <p className="text-xs text-muted-foreground mt-1">
                    EmaReach Provider works best with easy setup and managed SendGrid delivery.
                  </p>
                  {smtpProvider === "emareach" && (
