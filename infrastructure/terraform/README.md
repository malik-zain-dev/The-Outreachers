# Terraform – Emareach Backend on AWS
...
1. **Get an ACM certificate** (same region as the ALB, e.g. `us-east-1`):
   - **Option A:** AWS Console → **Certificate Manager** → Request a public certificate → add your domain (e.g. `api.emareach.com`) → validate via DNS.
...
2. **Point your domain to the ALB:** In your DNS provider, add a **CNAME**: name = your API subdomain (e.g. `api-emareach`), value = ALB DNS name from `terraform output alb_dns_name`.

3. **Set in `terraform.tfvars`:**
   ```hcl
   enable_https         = true
   acm_certificate_arn  = "arn:aws:acm:us-east-1:YOUR_ACCOUNT:certificate/YOUR_CERT_ID"
   domain_name          = "api.emareach.com"
   ```

4. **Apply:** `terraform apply`. The ALB gets an HTTPS listener on 443 and HTTP (80) redirects to HTTPS.

5. **Use the HTTPS URL** in frontend/admin: `NEXT_PUBLIC_API_URL=https://api.emareach.com/api`, `VITE_API_URL=https://api.emareach.com/api`.

## Tracking domain setup (Phase 1)

Use a separate host for tracking links/pixels (recommended: `track.emareach.com`).

1. Add DNS:
   - `track.emareach.com` CNAME -> `api.emareach.com` (or directly to ALB DNS).
2. Ensure your ACM cert includes `track.emareach.com` (or wildcard `*.emareach.com`).
3. Set backend env:
   - `TRACKING_BASE_URL=https://track.emareach.com`
4. Optional custom-domain verification target:
   - `TRACKING_CNAME_TARGET=track.emareach.com`

## Troubleshooting: https://api.emareach.com/ not loading
...
- **CNAME record:** Hostinger (or your DNS) must have:
  - **Name/host:** `api-emareach` (so full name is `api.emareach.com`)
  - **Target/value:** `emareach-backend-production-alb-1093121194.us-east-1.elb.amazonaws.com`  
    **No** `https://` or trailing `/` in the target.
- **Check resolution:**
  ```bash
  dig api.emareach.com CNAME +short
  # or: nslookup api.emareach.com
  ```
  You should see the ALB hostname. If not, fix the CNAME and wait for DNS propagation (up to 48h, often minutes).

### 2. ACM certificate

- The ALB HTTPS listener uses the cert in `acm_certificate_arn`. That cert must **include** `api.emareach.com` (or `*.aikaptan.com`).
- **Check:** AWS Console → **Certificate Manager** (us-east-1) → open the certificate used in `terraform.tfvars` → under "Domain name" you should see `api.emareach.com` or `*.aikaptan.com`, and status **Issued**.
- If the cert is for a different name, request a new cert for `api.emareach.com`, validate via DNS, then set `acm_certificate_arn` in `terraform.tfvars` and run `terraform apply`.
...
- **HTTP (should redirect to HTTPS):**  
  `curl -I http://api.emareach.com`  
  Expect `301` or `302` to `https://...`.
- **HTTPS:**  
  `curl -I https://api.emareach.com/api/health`  
...
If a new EC2 instance has no `emareach-backend` container (`docker logs emareach-backend` → "No such container"):
...
4. **Ensure ECR has the image:** Push from main (or run the deploy workflow) so `emareach-backend:latest` exists in ECR; the instance role must have permission to pull from ECR.
...
| `project_name` | No | `emareach-backend` | Prefix for resource names. |
...
| `domain_name` | No | `""` | Custom domain (e.g. `api.emareach.com`) for `app_url` output. Point CNAME to ALB. |
